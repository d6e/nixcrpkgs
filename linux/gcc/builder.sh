...
cd ..
make -j$NIX_BUILD_CORES -C build_gcc all-gcc
mkdir build_musl
cd build_musl
...
cd ..
make -j$NIX_BUILD_CORES -C build_musl install-headers
make -j$NIX_BUILD_CORES -C build_gcc all-target-libgcc
make -j$NIX_BUILD_CORES -C build_musl
make -C build_musl install
make -j$NIX_BUILD_CORES -C build_gcc
